...
void BM_Computing_bit_cpu(benchmark::State& st)
{
    std::vector<char> data(height * width);

    for (auto _ : st)
        bit_cpu(width, height, n_iterations);

    st.counters["frame_rate"] = benchmark::Counter(st.iterations(),
                                                   benchmark::Counter::kIsRate);
}

void BM_Computing_bit_cpu_parallel(benchmark::State& st)
{
    std::vector<char> data(height * width);

    for (auto _ : st)
        bit_cpu_parallel(width, height, n_iterations);
...
/*
BENCHMARK(BM_Computing_bit_cpu)
->Unit(benchmark::kMillisecond)
->UseRealTime();

BENCHMARK(BM_Computing_bit_cpu_parallel)
->Unit(benchmark::kMillisecond)
->UseRealTime();
*/
